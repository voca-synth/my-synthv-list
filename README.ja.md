...
公開できたら、ぜひシェアしてください。このリポジトリにリンク付きの issue を立ててもらえれば、ほかの人にも見つけてもらえます。紹介したクリエイターに送るのもおすすめです。自分の作品を好きな人がいると知って、喜んでもらえることが多いはずです。サイト自体の改善は pull request で歓迎します。公開する前に[免責事項](#免責事項)を読んでください。リストで共有する内容の責任は作成者にあります。
...
7曲目には投稿者によるコンテンツ警告があります。視聴前に動画の説明欄を確認してください。4曲目は投稿者により限定公開になっているため、このリストからは再生できますが、YouTube の検索には表示されません。クレジットは 2026年9月時点の動画説明欄と原曲の投稿から取得しました。動画はいつでも削除または非公開になる可能性があります。クリエイターの方で、作品をこのリストから外してほしい場合は、issue または `scuba-had-goofy[at]duck.com` までお知らせください。

このプロジェクトを使ってほかの人が作ったリストは、その作成者のものです。私たちはそれらを確認、ホスト、承認しておらず、その内容について一切の責任を負いません。この issue でリンクが共有された場合も同様です。リストを作る場合は、共有する内容を自分の判断と責任で確認してください。動画が公式または許可を得たアップロードであること、クレジットが正しいこと、必要なコンテンツ警告を付けていること、YouTube の利用規約とお住まいの地域の法律を守っていることなどです。コードはライセンスに記載のとおり現状のまま提供され、いかなる保証もありません。

## ライセンス

Copyright © 2026 Voca Synth（[github.com/didvc](https://github.com/didvc)）。このサイトのコードは [GNU AGPL-3.0](LICENSE)（SPDX：`AGPL-3.0-only`）でライセンスされています。このライセンスは、掲載している楽曲、動画、イラストには適用されません。ライセンスの範囲、外部サービス、商標については [NOTICE](NOTICE) を参照してください。

連絡先：issue を立てるか、`scuba-had-goofy[at]duck.com` までメールしてください。
